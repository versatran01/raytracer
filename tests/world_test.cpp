...
SCENARIO("Shading an intersection") {
  ...
  const auto i = shape.MakeIntersection(4);
...
SCENARIO("Shading an intersection from the inside") {
  ...
  const auto i = shape.MakeIntersection(0.5);
  const Computations comps(i, r);
...
SCENARIO("The reflected color from a nonreflective material") {
  auto w = DefaultWorld(1.0);
  const Ray r{Point3::Zero(), Vector3::UnitZ()};
  const auto& shape = w.shapes.back();
  const auto i = shape.MakeIntersection(1.0);
  const Computations comps(i, r);
...
SCENARIO("The refracted color with an opaque surface") {
  auto w = DefaultWorld();
  const auto& shape = w.shapes.front();
  const Ray r{Point3(0, 0, -5), Vector3(0, 0, 1)};
  const Intersections xs{shape.MakeIntersection(4), shape.MakeIntersection(6)};
